<!-- transcribed to markdown with pandoc -->
...
0.  报名要求：
    ...
    2.  有录制较清晰手元的能力。（如采用录屏需要staff担保）

    3.  高级组选手拥有自己赛区内80%以上谱面且必须拥有该区所有定数≥10.0的谱面（BYD铺面只要求主线）。

    4.  初级组选手拥有自己赛区内60%以上谱面且拥有该区80%以上定数在10.0~11.3之间（包括10.0及11.3）的谱面。

1.  选手数：高级组和初级组各48人（预计）
...
2.  赛制
    ...
    1.  本次赛事将选手和歌曲分为3个赛区：速度/S、技巧/T、节奏/Cb
        ...
        当满足以下条件其一时，不得报名初级组。但不满足以下条件的选手仍可报名高级组。

        ptt≥12.70/PM该赛区任意定数≥10.8的谱面/在该赛区任意定数≥9.8的铺面中达成理论值-10或更高分
    ...
    5.  第二轮：8进4个人赛
        ...
        流程：每位选手ban三首曲目→裁判公示禁用曲目→每位选手pick两首曲目+裁判random一首曲目，共5首。总分高者晋级下一轮。

        本轮的选曲范围是定数9.6～11.5的曲目
...
3.  计分方式
    ...
    S1组：当单曲分数-对手单曲分数≥10,000时计分公式变化为：（Maxpure\*1+Pure\*0.96+Far\*0.2）/Note总数

    \[S2组：当单曲分数-对手单曲分数≥20,000时计分公式变化为：（Maxpure\*1+Pure\*0.95+Far\*0.2）/Note总数]{.mark}

    T1组：当单曲Lost数＜对手Lost数时计分公式变化为：（Maxpure\*1+Pure\*0.9+Far\*0.5）/Note总数

    \[T2组：当对手Lost数-单曲Lost数≥2时计分公式变化为：（Maxpure\*1+Pure\*0.9+Far\*0.45）/Note总数]{.mark}

    Cb1组：当单曲小p数-对方小p数≥10且单曲分数≥9,970,000时计分公式变化为：（Maxpure\*1+Pure\*0.9+Far\*0.2+Lost\*0.75）/Note总数

    \[Cb2组：当单曲小p数-对方小p数≥20且单曲分数≥9,950,000时计分公式变化为：（Maxpure\*1+Pure\*0.9+Far\*0.2+Lost\*0.7）/Note总数]{.mark}
